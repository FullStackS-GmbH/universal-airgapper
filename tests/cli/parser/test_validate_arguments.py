...
class MockArgs:
    def __init__(
        self, credentials_file=None, credentials_folder=None, config_file=None, config_folder=None
    ):
...
def test_validate_arguments_config_missing():
    ...
    with pytest.raises(
        ValueError, match="You must provide exactly one of --config-file or --config-folder."
    ):
